...
$foto = mysqli_query ($conn, "SELECT * FROM foto WHERE user_id = $user_id");
...
<center>
<h4 style="margin-left:70px;">Galeri <u><?=$nama?> ! </u></h4><br>
...
<div class="d-flex flex-row" style="margin-left:90px; width:25%; float:left;">
    ...
    <div class="card shadow-sm p-3 mb-5 bg-body rounded" style="width:500px; height:550px;">
        ...
        <?= $row["tanggal_unggah"]; ?><br><br>
            ...
            <a href="like.php?foto=<?= $row["foto_id"]; ?>&user=<?=$user_id?>" class="btn btn-outline-danger">💔 </a>
...
<br><br>
            ...
            <?php
                if($row["user_id"] === $user_id){ ?>
                    <a href="hapus.php?foto=<?= $row["foto_id"]; ?>&user=<?=$user_id?>" onclick="return confirm('Anda Yakin?');" class="btn btn-outline-secondary">hapus</a>
            <?php    }
            ?>
        </div>
    </div>
